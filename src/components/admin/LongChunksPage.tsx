...
export default function LongChunksPage() {
  ...
  const doSplitPreview = async () => {
    ...
    try {
      ...
      }>('/api/kb/chunk/split-preview', {
        memoryId: splitMemory.id,
        instruction: splitInstruction.trim() || undefined,
      })
  ...
  return (
    <div style={{ display: 'flex', flex: 1, gap: 16, height: '100%', minHeight: 0 }}>
      ...
      <Modal
        ...
        okText="确认拆分"
        okButtonProps={{ disabled: splitPreviewLoading || splitChunks.length === 0 }}
      >
          ...
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            <div>
              ...
              <Space.Compact style={{ width: '100%', marginTop: 4 }}>
                ...
                <Button
                  type="primary"
                  icon={splitChunks.length > 0 ? <ThunderboltOutlined /> : <SearchOutlined />}
                  onClick={doSplitPreview}
                  loading={splitPreviewLoading}
                  style={{ height: 'auto', minHeight: 44 }}
                >
                  {splitChunks.length > 0 ? '重新拆分' : '获取拆分建议'}
                </Button>
              </Space.Compact>
            </div>
            ...
            {!splitPreviewLoading && splitChunks.length > 0 && (
              <>
                <Alert
                  type="info"
                  showIcon
